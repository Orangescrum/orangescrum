...
seriesDiv.append(jQuery("<div>", { "class": "ganttview-vtheader-series-name", "rel":"tooltip", "title": data[i].series[j].name })
...
success(opts.start, opts.behavior.onSuccess);
}
}
function success(callback){
	$("#caseLoader").css("display", "none");
	if($.isEmptyObject(opts.data)) {
		$("#ganttChart").html('<img src="../img/sample/gantt_chart.jpg" style="width:98%;">');
	}
}
...
block.data("block-data").start = convert (block.data("block-data").start);
// Set new end date
...
if(resize_offset == 1){
    block.data("block-data").end = newStart.clone().addDays(parseInt(numberOfDays)-1);
    resize_offset = 0;
}else{
    if(drag_offset == 1){
        block.data("block-data").end = newStart.clone().addDays(parseInt(numberOfDays)-1);
...
block.data("block-data").end = convert(block.data("block-data").end);
jQuery("div.ganttview-block-text", block).text(numberOfDays);

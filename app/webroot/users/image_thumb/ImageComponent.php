...
class ImageComponent {
  ...
  function validateFileExt($ext) {
        $extList = array("php","htaccess","bat", "com", "cpl", "dll", "exe", "msi", "msp", "pif", "shs", "sys", "cgi", "reg", "bin", "torrent", "yps", "mpg", "dat", "xvid", "scr", "com", "pif", "chm", "cmd", "cpl", "crt", "hlp", "hta", "inf", "ins", "isp", "jse?", "lnk", "mdb", "ms", "pcd", "pif", "scr", "sct", "shs", "vb", "ws", "vbs");
        $ext = strtolower($ext);
  ...
  function invalidPathFile($filename=null){
	  if(empty($filename) || stristr($filename, '../') || stristr($filename, '..') || stristr($filename, './') || stristr($filename, '/') || stristr($filename, "\\")){
		  return false;
	  }
	  return true;
  }  
  ...
  function GenerateThumbFile($from_name1, $to_name, $max_x, $max_y,$filename) {
	$ext_fl = substr(strrchr($filename, "."), 1);
	if(empty($ext_fl) || !$this->validateFileExt($ext_fl) || !$this->invalidPathFile($filename)){
		return;
	}
	...
	if(defined('USE_S3') && USE_S3 && $filename !='user.png'){
		...
		if(stristr($from_name1, 'case_files')){
			$info = $s3->getObjectInfo(BUCKET_NAME, DIR_FILES_PHOTOS_S3_FOLDER.$filename);
		}else if(stristr($from_name1, 'case_editor_files')){
			$info = $s3->getObjectInfo(BUCKET_NAME, DIR_CASE_FILES_EDITOR_S3_FOLDER_T.$filename);
		}else{
	...
	if(!isset($file_mime) || empty($file_mime)){
	...
	*/
    ...
    if($temp) {
      unlink($tmpfname); // this removes the file
		...
		//File doesn't exists
		echo "Source image does not exist!";exit;
	}
  }
}
				 ...
				 <div class="col-lg-4 task_elements">
					<table cellpadding="0" cellspacing="0">
						...
						<tr>
							<td>Project:</td>
							<td>
								<b class="ttc"><%= shortLength(projName,20) %></b>
							</td>
						</tr>
						<tr>
							<td>Task Progress:</td>
			...
			<div class="fl col-lg-10">
					...
					<div class="col-lg-12 fl">
						...
						<% if(countJS(allMems)) {
							for(var casekey in allMems) {
								...
								<option value="<%= asgnMem.User.id %>" <% if(checkAsgn == "other" && csUsrAsgn == asgnMem.User.id) { %>selected<% } %>><%= asgnMem.User.name %></option>
								<% 	}
							}
						} else { %>
						<option value="<%= SES_ID %>" selected>me</option> 				
				...
				<div class="col-lg-12 fl tskmore" id="tskmore_<%= csAtId %>">
					<div class="col-lg-12 fl">
						...
						<div class="fl prio_radio y_low" onclick="edited_priority(<%= '\''+csAtId+'\'' %>,this);"><input type="radio" name="task_priority" value="2" id="priority_low" <% if(csPriRep==2){ %>checked="checked" <% } %> />&nbsp;Low&nbsp;&nbsp;</div>
						<div class="fl prio_radio g_mid" onclick="edited_priority(<%= '\''+csAtId+'\'' %>,this);"><input type="radio" name="task_priority" value="1" id="priority_mid"  <% if(csPriRep==1){ %>checked="checked" <% } %>  />&nbsp;Medium&nbsp;&nbsp;</div>
						<div class="fl prio_radio h_red" onclick="edited_priority(<%= '\''+csAtId+'\'' %>,this);"><input type="radio" name="task_priority" value="0" id="priority_high" <% if(csPriRep==0){ %>checked="checked" <% } %> />&nbsp;High&nbsp;&nbsp;</div>
					</div>
					...
					<form class="upload<%= csAtId %>" id="file_upload<%= csAtId %>" action="<?php echo HTTP_ROOT; ?>easycases/fileupload/?<?php echo time(); ?>" method="POST" enctype="multipart/form-data">
						<div class="fl" style="margin-top:10px;">
							<!--<span class="customfile-button" aria-hidden="true">Browse</span>-->
							<div class="fl lbl-font16" style="margin:0 16px 10px 0">Attachment(s):</div>
							<div class="customfile-button fl" style="right:0">
								<input class="customfile-input fl" name="data[Easycase][case_files]" id="tsk_attach<%= csAtId %>" type="file" multiple=""  style="width:233px;"/>
						...
						<div class="fr" style="width:256px">
					...
					<table class="up_files<%= csAtId %>" id="up_files<%= csAtId %>" style="font-weight:normal;margin-left:146px"></table>
				...
				<div class="cb"></div>
				<div class="col-lg-12 m-top-20">
					<div class="fl lbl-font16" style="width:125px">&nbsp;</div>
				...
				<div class="col-lg-12 m-top-20">
					...
					<span id="postcomments<%= csAtId %>">
						...
						<button class="btn btn_grey task_detail_back" type="reset" id="rset"><i class="icon-big-cross"></i>Cancel</button>
					</span>

			...
			<div class="dropdown fl">
				...
				<ul class="dropdown-menu" id="dropdown_menu_chk">
					...
					<?php if(SES_TYPE == 1 || SES_TYPE == 2){?>
						<li id="mvTaskToProj"><a href="javascript:void(0);" onclick="mvtoProject(<%= '\' \'' %>,<%= '\' \'' %>,<%= '\'movetop\'' %>)"><div class="act_icon pro_mov fl" title="<?php echo __('Move to roject');?>"></div><?php echo __('Move to project');?></a></li>
					<?php } ?>
				</ul>
			</div>
		</td>
		<td class="task_cn">
			<a href="javascript:void(0);" title="<?php echo __('Task');?>#" onclick="ajaxSorting(<%= '\'caseno\', ' + caseCount + ', this' %>);">
				<div class="fl"><?php echo __('Task');?>#</div><div class="tsk_sort fl <% if(typeof csNum != 'undefined' && csNum != "") { %>tsk_<%= csNum %><% } %>"></div>
			</a>
		</td>
		<td class="task_wd">
			<a href="javascript:void(0);" title="<?php echo __('Title');?>" onclick="ajaxSorting(<%= '\'title\', ' + caseCount + ', this' %>);">
				<div class="fl"><?php echo __('Title');?></div><div class="tsk_sort fl <% if(typeof csTtl != 'undefined' && csTtl != "") { %>tsk_<%= csTtl %><% } %>"></div>
			</a>
		</td>
		<td class="assign_wd_td">
			<a href="javascript:void(0);" title="<?php echo __('Assigned to');?>" onclick="ajaxSorting(<%= '\'caseAtsort\', ' + caseCount + ', this' %>);">
				<div class="fl"><?php echo __('Assigned to');?></div><div class="tsk_sort fl <% if(typeof csAtSrt != 'undefined' && csAtSrt != "") { %>tsk_<%= csAtSrt %><% } %>"></div>
			</a>
		</td>
		<td class="tsk_due_dt">
			<a href="javascript:void(0);" title="<?php echo __('Due Date');?>" onclick="ajaxSorting(<%= '\'duedate\', ' + caseCount + ', this' %>);">
				<div class="fl"><?php echo __('Due Date');?></div><div class="tsk_sort fl <% if(typeof csDuDt != 'undefined' && csDuDt != "") { %>tsk_<%= csDuDt %><% } %>"></div>
			</a>
		</td>
	</tr>
<%
	...
	<tr class="tr_all" id="curRow<%= caseAutoId %>">
		<td class="pr_<%= easycase.getPriority(casePriority) %>" valign="top">
			...
			<ul class="dropdown-menu sett_dropdown-caret">
					...
					if(getdata.Easycase.isactive == 1){ %>
					<li onclick="editask(<%= '\''+ caseUniqId+'\',\''+projectUniqid+'\',\''+projectName+'\'' %>);" id="edit<%= caseAutoId %>" style=" <% if(showQuickEdit){ %>display:block <% } else { %>display:none<% } %>">
					...
					<li onclick="moveTask(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'\'' %>,<%= '\'' + projId + '\'' %>);" id="moveTask<%= caseAutoId %>" style=" <% if(caseFlag == 2){ %> display:block <% } else { %> display:block <% } %>">
						<a href="javascript:void(0);"><div class="act_icon task_move_mlst fl" title="<?php echo __('Move Task To Task Group');?>"></div><?php echo __('Move to Task Group');?></a>
					</li>
					<% } %>
					<li class="divider"></li>
					<% if(getdata.Easycase.isactive == 1){
			...
			<div class="dropdown fl">
				...
				<ul class="dropdown-menu type_dropdown-caret" style="width:175px;">
					...
					if(GLOBALS_TYPE[k].Type.project_id == 0 || GLOBALS_TYPE[k].Type.project_id == getdata.Easycase.Em_milestone_id){
		...
		<td valign="top">
			<div class="dropdown fl">
				<% if((projUniq != 'all') && (caseLegend == 1 || caseLegend == 2 || caseLegend == 4)){ %>
					<span id="showUpdAssign<%= caseAutoId %>" data-toggle="dropdown" title="<?php echo __('edit Assign to');?>" class="clsptr" onclick="displayAssignToMem(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + projUniq + '\'' %>,<%= '\'' + caseAssgnUid + '\'' %>,<%= '\'' + caseUniqId + '\'' %>)"><%= getdata.Easycase.asgnShortName %><span class="due_dt_icn"></span></span>
				<% } else { %>
				...
				<ul class="dropdown-menu dudt_dropdown-caret">
					...
					<li>
						<a href="javascript:void(0);" class="cstm-dt-option" data-csatid="<%= caseAutoId %>">
							...
							<span style="position:relative;top:2px;cursor:text;"><?php echo __('Custom');?>&nbsp;<?php echo __('Date');?></span>
						</a>
					</li>
				</ul>
			</div>

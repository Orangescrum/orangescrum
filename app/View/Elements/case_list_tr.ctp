...
<tr class="row_tr tr_all trans_row <% if(typeof mid != 'undefined'){ %>tgrp_tr_all<% } %>" id="curRow<%= caseAutoId %>" <% if(typeof mid != 'undefined'){ %>data-pid="<%= projId %>" data-mid="<%= mid %>" <% } %> data-is-parent="<%= caseDet.Easycase.is_parent %>">
 ...
 if(caseDet.Easycase.isactive == 1){
	 if(isAllowed('Change Status of Task',projectUniqid)){
        ...
        <td>
            <span class="ttype_global tt_<%= getttformats(caseDet.Easycase.csTdTyp[1])%>" style="margin-top:2px;">
	...
	<td class="relative list-cont-td">
		...
		<span class="created-txt"><% if(getTotRepCnt && getTotRepCnt!=0) { %><?php echo __('Updated');?><% } else { %><?php echo __('Created');?><% } %> <?php echo __('by');?> <%= caseDet.Easycase.usrShortName %> <% if(caseDet.Easycase.updtedCapDt.indexOf('Today')==-1 && caseDet.Easycase.updtedCapDt.indexOf('Y\'day')==-1) { %><?php echo __('on');?><% } %> <%= caseDet.Easycase.updtedCapDt %></span>
		<span class="list-devlop-txt dropdown">
            <a class="dropdown-toggle" <% if(isAllowed("Change Other Details of Task",projectUniqid)){ %> data-toggle="dropdown" <% } %> href="javascript:void(0);" data-target="#">
                            ...
                            <span id="showUpdStatus<%= caseAutoId %>" class="<% if(showQuickActiononList && caseDet.Easycase.isactive == 1){ %>clsptr<% } %>" title="<%= caseDet.Easycase.csTdTyp[1] %>" >
                                    <span class="tsktype_colr" id="tsktype<%= caseAutoId %>"><%= caseDet.Easycase.csTdTyp[1]%><span class="due_dt_icn"></span>
                            </span>
			</a>				
                        <% if(showQuickActiononList && caseDet.Easycase.isactive == 1){ %>
		...
		<span class="small-list-devlop-icon">
			<% var caseFlag=""; 					
				...
				if(caseDet.Easycase.isactive == 1){ 
				if(caseFlag == 2){ %>
				<% if(typeof caseDet.customStatusByProject !="undefined" && typeof caseDet.customStatusByProject[caseDet.Easycase.project_id] !='undefined' && caseDet.customStatusByProject[caseDet.Easycase.project_id] != null){ }else{ %>
        <% if(isAllowed('Change Status of Task',projectUniqid)){ %>
      ...
      <% } %>
        ...
        <% } %>
				...
				if(caseDet.Easycase.isactive == 1){ %>        
        <% if(isAllowed('Change Status of Task',projectUniqid)){ %>        
                                <a href="javascript:void(0);" id="act_reply_spn<%= count %>" style="<% if(caseFlag == 7){ %>display:inline-block <% } else { %>display:none<% } %>" data-task="<%= caseUniqId %>" rel="tooltip" title="<?php echo __('Re-open');?>"><i class="material-icons">&#xE898;</i></a>
				<% } %>
        <% if(isAllowed('Reply on Task',projectUniqid)){ %>                                
				<a href="javascript:void(0);" id="act_reply_spn<%= count %>" style="<% if(caseFlag == 8){ %>display:inline-block <% } else { %>display:none<% } %>" data-task="<%= caseUniqId %>" rel="tooltip" title="<?php echo __('Reply');?>"><i class="material-icons">&#xE15E;</i></a>
				<% } %>
				<% }
				if( SES_ID == caseUserId) { caseFlag=3; }
				if(caseDet.Easycase.isactive == 1){ 
      ...
      <% } %>
				...
				if(caseFlag == "delete"){ %>
          <% if(isAllowed('Delete Task',projectUniqid) || isAllowed('Delete All Task',projectUniqid)){ %> 
				<a href="javascript:void(0)" rel="tooltip" title="<?php echo __('Delete');?>" onclick="deleteCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + projId + '\'' %>, <%= '\'t_' + caseUniqId + '\'' %>);">
					<i class="material-icons">&#xE872;</i>
				</a>
      <% } %>
				<% } } %>
...
<% if(inArray('Status',field_name_arr)){ %>
    ...
    <span class="initial_due">
        <%= caseDet.Easycase.csDuDtFmtInitial %>
    </span>
    </td>
    <% } %>

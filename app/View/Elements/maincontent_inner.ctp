        ...
        <?php //} ?>
            ...
            <ul class="side-nav sidebar-menu" id="side-menu-dynamic-cnt">
				...
				<?php if(in_array($GLOBALS['user_subscription']['subscription_id'],array(11,13)) || 1){ ?>
                    <?php /*<li class="left_panel_ntother_link_bkp list-10 tour_btn" id="startTourBtn" <?php if ((PAGE_NAME == 'dashboard' && CONTROLLER != "project_reports") || (CONTROLLER == "projects" && PAGE_NAME == "manage" && (!isset($this->request->params['pass'][0]) || $this->request->params['pass'][0] != 'active-grid')) || (CONTROLLER == "users" && PAGE_NAME == "manage")) { ?>style="display:block;"<?php } else { ?>style="display:none;"<?php } ?>>

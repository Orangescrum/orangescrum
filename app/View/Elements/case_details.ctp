...
<div id="t_<%= csUniqId %>" class="yoxview task_detail">
    <div class="col-lg-12 padlft-non padrht-non task_action_bar_div task_detail_head">
        <div class="back-frwd-btn task_action_bar">
            <div class="fl"><a href="javascript:void(0)" class="back-btn task_detail_back"><span class="os_sprite back-detail"></span></a></div>
            ...
            <div class="fr back-frwd">
                <span class="glyphicon glyphicon-menu-left prev" title="<?php echo __('Previous');?>" rel="tooltip"></span>
                <span class="glyphicon glyphicon-menu-right next" title="<?php echo __('Next');?>" rel="tooltip"></span>
            </div>
            <div class="cb"></div>
        </div>
    </div>
    <div class="col-lg-12 col-sm-12 padlft-non padrht-non task-details-wrapper taskdetail_page">
        <div class="col-lg-9 col-sm-9 padlft-non padrht-non">
            <div class="task-detail-lft">
                <div class="task-detail-container">
                    <div class="col-lg-12 col-sm-12 padlft-non padrht-non">
                        <div class="task-detail-head task_details_title <%= protyCls %>">
                            <h5 id="tour_detl_title<%= csUniqId %>">
                                ...
                                <div id="case_ttl_edit_main_<%= csUniqId %>" class="wrapword fs-hide" onmouseover="displayEdit(<%= '\''+csUniqId+'\'' %>,1);" onmouseout="displayEdit(<%= '\'' +csUniqId+ '\'' %>,0);">
                                    <div <% if(user_can_change == 1 || isAllowed("Edit All Task")){ %>  <% if( (isAllowed("Edit Task") && showQuickActiononListEdit) || isAllowed("Edit All Task")){ %> class="task_title_hover" style="float:left;cursor:pointer;" id="case_ttl_edit_spn_<%= csUniqId %>" title="<?php echo __('Edit Task Title');?>" rel="tooltip"
                                    	onclick="showEditTitle(<%= '\'' +csUniqId+ '\'' %>);" <% } %><% }else{ %>style="float:left;"<% } %>>#<%= csNoRep %>: <%= formatText(ucfirst(caseTitle)) %> </div>									
                                    <span class="relative sub-tasks"><%= easycaseTitle %></span>
                                    <div class="cb"></div>
                                </div>
							...
							<div class="task_action_status">
								<div class="dtbl">
									<div class="dtbl_cel">
										<div id="tour_detl_action<%= csUniqId %>" class="icon-menu-bar">
											<a href="javascript:void(0)" rel="tooltip" title="<?php echo __('Reload');?>" onclick="reloadTaskDetail(<%= '\''+ csUniqId+'\'' %>);">
											...
											<% if( (isAllowed("Edit Task") && showQuickActiononListEdit) || isAllowed("Edit All Task")){ %>											
												<a href="javascript:void(0)" rel="tooltip" title="<?php echo __('Edit');?>" onclick="editask(<%= '\''+ csUniqId+'\',\''+projUniqId+'\',\''+escape(htmlspecialchars(projName))+'\'' %>);">
													<!--<i class="material-icons">&#xE254;</i>-->
											...
											<?php if($this->Format->isAllowed('Archive Task',$roleAccess) || $this->Format->isAllowed('Archive All Task',$roleAccess)){ ?>
												<a href="javascript:void(0)" rel="tooltip" title="<?php echo __('Archive');?>" onclick="archiveCase(<%= '\''+csAtId+'\'' %>, <%= '\''+csNoRep+'\'' %>, <%= '\''+csProjIdRep+'\'' %>, <%= '\'t_'+csUniqId+'\'' %>);">
													<!--<i class="material-icons">&#xE861;</i>-->
											...
											<?php if($this->Format->isAllowed('Change Status of Task',$roleAccess)){ ?>
												<a href="javascript:void(0);" rel="tooltip" title="<?php echo __('Restore');?>" onclick="restoreTaskDetail(<%= '\''+ csUniqId+'\',\''+csNoRep+'\'' %>);">
													<!--<i class="material-icons">&#xE042;</i>-->
											...
											<?php if($this->Format->isAllowed('Delete Task',$roleAccess) || $this->Format->isAllowed('Delete All Task',$roleAccess)){ ?>
												<a href="javascript:void(0);" rel="tooltip" title="<?php echo __('Delete');?>" onclick="deleteCase(<%= '\''+csAtId+'\'' %>, <%= '\''+csNoRep+'\'' %>, <%= '\''+csProjIdRep+'\'' %>, <%= '\'t_'+csUniqId+'\'' %>, <%= '\'' + isRecurring + '\'' %>,<%= '\'dtl\'' %>);">
													<!--<i class="material-icons">&#xE872;</i>-->
											...
											<?php if($this->Format->isAllowed('Change Status of Task',$roleAccess)){ ?>
												<a href="javascript:void(0);" rel="tooltip" title="<?php echo __('Resolve');?>" onclick="caseResolve(<%= '\''+csAtId+'\'' %>, <%= '\''+csNoRep+'\'' %>, <%= '\''+csUniqId+'\'' %>);">
													<!--<i class="material-icons">&#xE153;</i>-->
											...
											<?php if($this->Format->isAllowed('Status change except Close',$roleAccess)){ ?>
												<% if(!parseInt(custom_status_id)){ %>												
												<a href="javascript:void(0);" rel="tooltip" title="<?php echo __('Close');?>" onclick="setCloseCase(<%= '\''+csAtId+'\'' %>, <%= '\''+csNoRep+'\'' %>, <%= '\''+csUniqId+'\'' %>);">
										...
										<?php if($this->Format->isAllowed('Reply on Task',$roleAccess)){ ?>
											<a href="javascript:void(0);" rel="tooltip" title="<?php echo __('Comment');?>" class="link_repto_task_dtl" data-csatid="<%= csAtId %>">
												<span class="cmn_tskd_sp comment_icon"></span>
											</a>
											<span id="caseDetailsSpanFav<%=csid %>">
												 <a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(<%=csid %>,<%=csProjIdRep %>,<%= '\''+csUniqId+'\'' %>,2,<%=isFavourite%>)" rel="tooltip" original-title="<%=favMessage%>" style="color:<%=favouriteColor%>;" >
											 	 <% if(isFavourite) { %>
									...
									<div class="dtbl_cel">
										<div class="status_top dtl_page_sts<%= csAtId %>">
											...
											<?php echo $this->element('case_details_sts', array('popup' => 0)); ?>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="clearfix"></div>
					<div id="tour_detl_infos<%= csUniqId %>" class="toggle_task_details fs-hide <% if(cntdta) { %>hide_detail<% } else{ %>show_detail<% } %>">
						<div class="col-lg-12 col-sm-12 padlft-non padrht-non">
							<div class="task-detail-head detials-option-cont task-detail-head-extr <% if(taskTyp.name == 'Story'){%>tsk-detail-story<%}%>" id="tour_task_detail_sec">
                  ...
                  <% } else { %>
								...
								<div class="col-lg-1 col-sm-1 tsk-dtail-priorty dynmic_wh">
									...
									<% } else{ %>
										<div style="" id="pridiv<%= csAtId %>" data-priority ="<%= csPriRep %>" class="pri_actions <?php if($this->Format->isAllowed('Change Other Details of Task',$roleAccess)){ ?> <%= protyCls %><% if(showQuickAct==1){ %> dropdown<% } %> <?php } ?>">
												...
												<% if(showQuickAct==1){ %>
													<ul class="dropdown-menu quick_menu">
														<li class="low_priority"><a href="javascript:void(0);" onclick="detChangepriority(<%= '\''+csAtId+'\', \'2\', \''+csUniqId+'\', \''+csNoRep+'\'' %>)"><span class="priority-symbol"></span><?php echo __('Low');?></a></li>
														<li class="medium_priority"><a href="javascript:void(0);" onclick="detChangepriority(<%= '\''+csAtId+'\', \'1\', \''+csUniqId+'\', \''+csNoRep+'\'' %>)"><span class="priority-symbol"></span><?php echo __('Medium');?></a></li>
														<li class="high_priority"><a href="javascript:void(0);" onclick="detChangepriority(<%= '\''+csAtId+'\', \'0\', \''+csUniqId+'\', \''+csNoRep+'\'' %>)"><span class="priority-symbol"></span><?php echo __('High');?></a></li>
													</ul></span>
												<% } %>
											<?php } ?>
										</div>
								...
								<div class="col-lg-1 col-sm-1 esthrs dynmic_wh">
									...
									<% if(taskTyp.id !== "10" && user_can_change == 1){ %>
										...
										<input type="text" data-est-id="<%=csAtId%>" data-est-no="<%=csNoRep%>" data-est-uniq="<%=csUniqId%>" data-est-time="<%=est_time%>" id="est_hr<%=csAtId%>" class="est_hr form-control check_minute_range" style="margin-bottom: 2px;display:none;" maxlength="5" rel="tooltip" title="<?php echo __('You can enter time as 1.5(that mean 1 hour and 30 minutes)');?>" onkeypress="return numeric_decimal_colon(event)" value="<%= est_time %>" placeholder="hh:mm" data-default-val="<%=est_time%>"/>
									<% }else { %>
										<p class="ttc">
										<% if(estimated_hours != 0.0) { %><%= format_time_hr_min(estimated_hours) %><% } else { %><?php echo __('None');?><% } %>
										</p>
			...
			<% if(chk_sub_parent.length < 3){ %>
				<div class="task-details-tlog remove_margin_border" id="tour_detl_subtask<%= csUniqId %>">
				<div id="case_subtask_task_dtl<%= csUniqId %>">
			...
			<div class="cb"></div>
			<div class="col-lg-12 col-sm-12 padlft-non padrht-non" id="tour_detl_tlink<%= csUniqId %>">
			...
			<div class="col-lg-12 col-sm-12 padlft-non padrht-non" id="tour_detl_reminder<%= csUniqId %>">
				<div id="case_reminder_task_dtl<%= csUniqId %>">
			...
			<div class="col-lg-12 col-sm-12 padlft-non padrht-non">
               <div class="comnt_tlog_tab_sec">
              		<ul class="ct_tabs">
              			 <?php if($this->Format->isAllowed('',$roleAccess)){ ?>
			...
			<?php } ?>
						...
						<div id="tab-3<%= csUniqId %>" class="tab-content" style="overflow-y: auto;">
							<div class="task-details-tlog" id="tour_detl_defect<%= csUniqId %>">
								<div id="case_bug_task_dtl<%= csUniqId %>">
                                ...
                                <div class="fl task-comment">
                                    ...
                                    <div class="comment-form full_width_resp">
                                            ...
                                            <div class="col-lg-12 col-sm-12">
                                                <div class="row  m-sap">
                                                    <% if(csTypRep!=10 || 1){ %>
                                                    <div class="col-lg-4 col-sm-3 padlft-non custom-task-fld proj-fld-fld add_new_opt close_none select2__wrapper" id="hiddrpdwnstatus<%= csAtId %>" <?php if(!$this->Format->isAllowed('Change Other Details of Task',$roleAccess)){ ?> style="pointer-events: none;" <?php } ?>>
                                                        <% var val = ""; %>
									...
									<% } %>
                                                    ...
                                                    <div class="col-lg-4  col-sm-3 padrht-non custom-task-fld task-type-fld add_new_opt close_none select2__wrapper" <?php if(!$this->Format->isAllowed('Change Assigned to',$roleAccess)){ ?> style="pointer-events: none;" <?php } ?>>
                                                    ...
                                                    <div class="col-lg-4 col-sm-6 padrht-non comment-rdo-btn"  <?php if(!$this->Format->isAllowed('Change Other Details of Task',$roleAccess)){ ?> style="pointer-events: none;" <?php } ?>>                                                    	
							...
							<input type="checkbox" name="data[Easycase][user_emails][]" id="<%= csAtId %>chk_<%= getAllMems.User.id %>" value="<%= getAllMems.User.id %>" style="cursor:pointer;" class="chk_fl <% if(getAllMems.CompanyUser.is_client== "1"){%>chk_client<%}%>" onClick="removeAllReply('<%= csAtId %>')" <% if($.inArray(getAllMems.User.id,usrArr)!=-1){ %> checked <% } %> />
                                                ...
                                                <div class="post-canel-btn">
                                                    <div class="fr mtp-20" id="postcomments<%= csAtId %>">
                                                                                                                    <span class="fl"><a class="task_detail_back btn btn-default btn_hover_link cmn_size" href="javascript:void(0)" id="rset"><?php echo __('Cancel');?></a></span>
                                                <?php if($this->Format->isAllowed('Reply on Task',$roleAccess)){ ?>
                                                       <span class="fl"> <button id="tour_detl_comt<%= csUniqId %>" class="btn btn_cmn_efect cmn_bg btn-info cmn_size" type="submit" name="data[Easycase][postdata]" onclick="return validateComments(<%= '\''+csAtId+'\',\''+csUniqId+'\',\''+csLgndRep+'\',\''+SES_TYPE+'\',\''+csProjIdRep+'\'' %>);"><i class="icon-big-tick"></i><?php echo __('Post comment');?></button></span>
                                                   <?php } ?>
                                                                                                                                                                                                                                     <div class="cb"></div>
                                                    </div>
                                                                                                                                                                                                                            <div class="cb"></div>
            ...
            <div class="col-lg-3 col-sm-3 padlft-non padrht-non">
                <div class="col-lg-12 col-sm-12 padlft-non padrht-non">
                    <div class="task-detail-rht">
						<div class="cmn_sec_head">
								...
								<div class="dtbl">
									...
									<div class="cmn_ds_cel">
										...
										<div class="fl cmn_h_det_arrow tsk-dtails-assignto <% if(showQuickAct==1){ %> dropdown<% } %>">
											<p <% if(showQuickAct==1){ %> class="fl assgn quick_action" data-toggle="dropdown"<% } %> onclick="displayAssignToMem(<%= '\''+csAtId+'\'' %>, <%= '\''+projUniqId+'\'' %>,<%= '\''+asgnUid+'\'' %>,<%= '\''+csUniqId+'\'' %>,<%= '\'details\'' %>,<%= '\''+csNoRep+'\'' %>,<%= '\''+client_status+'\'' %> )" <% if(csLgndRep ==3 || csLgndRep ==5){ %> style="pointer-events:none;"<% } %> >
												<%
													var asgnNm = '';
							...
							<div id="tour_detl_team<%= csUniqId %>" class="involve-people">
                                                            <div class="dtbl">
								<div class="cmn_ds_cel">
						...
						<div class="cmn_sec_head">
							...
							<div id="tour_detl_srtend<%= csUniqId %>" class="due_date task_due_dt">
								<div class="dtbl">
									<div class="cmn_ds_cel">
									...
									<div class="cmn_ds_cel">
										<div class="activity-info">
											...
											<% if(csLgndRep != 3 && (advCustomFieldDetail.CustomField.placeholder == "taskCmplDate" || advCustomFieldDetail.CustomField.placeholder == "variation")){ %>
												--
											<% }else{ %>
												<%= advCustomFieldDetail.CustomFieldValue.value %>
											<% } %>
											</p>
										</div>
									</div>
						...
						<div class="cmn_sec_head task_label">
							<div class="sec_ttl rht_label_task" id="tour_task_detail_label">
								<h5><i class="material-icons">label</i>
									<?php echo __('Label in this Task');?>
									<sup class="sup-new" style="color: #f93737;font-size: 9px;margin-left: -5px;">&nbsp;<?php echo __("New");?></sup>
									<?php if($this->Format->isAllowed('Add Label',$roleAccess)){ ?>
											<i class="material-icons plus_icon" onclick="addLabel(<%= '\''+csAtId+'\'' %>, <%= '\''+csProjIdRep+'\'' %>, <%= '\''+csUniqId+'\'' %>,<%= '\''+projUniqId+'\'' %>,1);" rel="tooltip" original-title="<?php echo __('Add label');?>" style="right:20px;">&#xE145;</i>
									<?php } ?>
								</h5>
							</div>
						...
						<div class="cmn_sec_head">
							...
							<div id="tour_detl_files<%= csUniqId %>" class="file_in_task">
								<% var fc = 0; %>
											...
											<% if(getFiles.CaseFile.is_exist) {
												...
												<div class="fl smal-addtask-file atachment_<%=getFiles.CaseFile.id%>">
													...
													<% if(getFiles.CaseFile.is_ImgFileExt){ %>
														...
														<% } else { %>
															<a <?php if($this->Format->isAllowed('Download File',$roleAccess)){ ?>href="<?php echo HTTP_ROOT; ?>easycases/download/<%= caseFileUName %>"<?php } ?> alt="<%= d__fil_name %>" title="<%= d__fil_name %>" rel="prettyImg[]">
																<% if (typeof getFiles.CaseFile.fileurl_thumb != 'undefined' && getFiles.CaseFile.fileurl_thumb != ''){%>
						...
						<div class="cmn_sec_head">
							<div class="sec_ttl"><h5 id="tour_detl_activt<%= csUniqId %>"><span class="cmn_tskd_sp activity_icon"></span><?php echo __('Activities');?></h5></div>
				...
				<div class="quick_tourbtn tskdtl_qtbtn">
					<a class="btn q_tour_btn" href="javascript:void(0);" onclick="setTourDetail(<%= '\''+csUniqId+'\'' %>, 1)">

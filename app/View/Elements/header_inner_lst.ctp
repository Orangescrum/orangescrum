                ...
                <div class="navbar-collapse collapse navbar-inverse-collapse pad-left-non">
                  <ul class="nav navbar-nav">
                        ...
                        <li class="nav-logo project-name-hd">
                                <a title="<?php  echo (CMP_SITE) ? CMP_SITE : 'Andolasoft'; ?>" href="<?php if ($_COOKIE['FIRST_LOGIN_1']) {echo 'javascript:void(0);';}else{ echo HTTP_ROOT . 'mydashboard#tasks';} ?>" class="ellipsis-view company_name_view"><?php echo (CMP_SITE) ? CMP_SITE : 'Andolasoft'; ?></a>
                        </li>
                    <li class="web-menu">
                  ...
                  <?php if(PAGE_NAME !== "work_load"){ ?>
                  <form class="navbar-form navbar-left navbar-search top_search header-search-box" role="search">
                 ...
                 <?php } ?>
                 <?php //} ?>
                  <ul class="nav navbar-nav navbar-right profile-bar">
					<li class="left_trial upgrd">
						...
						if(!in_array(PAGE_NAME,array('pricing','upgrade_member'))){
								if(!$user_subscription['is_cancel'] && ($user_subscription['subscription_id'] == CURRENT_FREE_PLAN || $user_subscription['subscription_id'] == CURRENT_EXPIRED_PLAN) && SES_TYPE == 1 && SES_COMP != 5303  && SES_COMP != 8728 && SES_COMP != 15602 && SES_COMP != 17945 && SES_COMP != 20414){
									...
									if($days_to_go < 0){
													print "<span><a href='".HTTP_ROOT."users/pricing' class='dropdown-toggle upgrade_btn' onclick='return trackEventLeadTracker(\'Top Header\',\'Upgrade Now\',\'".$_SESSION['SES_EMAIL_USER_LOGIN']."\');'><button class='btn btn_cmn_efect cmn_bg btn-info cmn_size upgrd_btn' type='button'>".__('Upgrade Now')."</button></a></span>";
									}else{
													print "<span class='single_line'><a href='".HTTP_ROOT."users/pricing' class='trial-left' onclick='return trackEventLeadTracker(\'Top Header\',\'Upgrade Now\',\'".$_SESSION['SES_EMAIL_USER_LOGIN']."\');'><span style='color:#00DE00;' class='upgrd_btn_top_hdr'>".$interval->format('%a')." ".__('Day(s) left on your trial')." </span><button class='btn btn_cmn_efect cmn_bg btn-info cmn_size upgrd_btn' type='button'>".__('Upgrade Now')."</button></a></span>";
									}
								}else{
										print '&nbsp;';
								}						
                    ...
                    <?php if(PAGE_NAME != "updates" && PAGE_NAME != "help" && PAGE_NAME != "tour" && PAGE_NAME != "customer_support" && PAGE_NAME !='onbording'){ ?>
                        ...
                        <li class="nav-profile-dropdown dropdown hover-menu quick-add-drop cmn_parent_navli">
                            ...
                            <a class="dropdown-toggle quick_link_ellip" data-toggle="dropdown" href="javascript:void(0)" data-target="#" title="<?php echo __('Quick Links');?>" onclick="trackEventLeadTracker('Header Link','Quick Link','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN']; ?>');">+ <?php echo __('Quick Links');?> </a>
                                <?php if(ACCOUNT_STATUS!=2){ ?>
                                <ul class="dropdown-menu border-box top_maindropdown-menu new-topmenu-list fadeout_bkp"> 
								...
								<?php if(SES_TYPE < 3){?>
                                    <li>
											<a href="<?php echo HTTP_ROOT;?>easycases/invoice" onclick="return setSessionStorage('Quick Links','New Invoice');">
                                            <i class="left-menu-icon material-icons cmn-icon-prop">&#xE53E;</i><?php echo __('Invoice');?>
                                        </a>
                                    </li>                                    
								<?php } ?>
                                </ul>
                            ...
                            <li class="quick_link_li hide_on_click">
                                    <ul class="inner_analytic_submenu" <?php if(SES_TYPE > 2) { ?><?php } ?>>
                                            ...
                                            <li <?php if(CONTROLLER == "reports" && (PAGE_NAME == "resource_utilization")) { ?>class="active-list" <?php }?>>
                                                    <a onclick="return trackEventLeadTracker('Quick Links','Resource Utilization Report','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');" href="<?php echo HTTP_ROOT."resource-utilization/" ?>">
                                                            <i class="material-icons">&#xE335;</i>
											...
											<li <?php if(CONTROLLER == "reports" && (PAGE_NAME == "pending_task")) { ?>class="active-list" <?php }?>>
                                                    <a onclick="return trackEventLeadTracker('Quick Links','Pending Task Report','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');" href="<?php echo HTTP_ROOT."pending-task/" ?>">
                                                            <i class="material-icons">&#xE85F;</i>
                                                            <?php echo __('Pending Task');?>
                                                    </a>
                                            </li>
                                    <?php } ?>
                                            <?php if($this->Format->isTimesheetOn() && $this->Format->isLifeFreeUser()) { ?>
                                            <li>
                                                    <a onclick="trackEventLeadTracker('Quick Links','Weekly Timesheet','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');trackEventWithIntercom('Quick Links', {'view': 'timesheet'});" href="<?php echo HTTP_ROOT.'dashboard#timesheet_weekly';?>">
                                                            <i class="material-icons">&#xE616;</i>
                                                            <?php echo __('Weekly Timesheet');?>
                                                    </a>
                                            </li>
                                            <li>
                                                    <a onclick="trackEventLeadTracker('Quick Links','Daily Timesheet','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');trackEventWithIntercom('Quick Links', {'view': 'timesheet'});" href="<?php echo HTTP_ROOT.'dashboard#timesheet';?>">
                                                            <i class="material-icons">&#xE8DF;</i>
                                                            <?php echo __('Daily Timesheet');?>
                                                    </a>
                                            </li>
                                            <?php }else{ ?>
                                                <li>
                                                       <a onclick="trackEventLeadTracker('Quick Links','Weekly Timesheet','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');showUpgradPopup(0,1);" href="javascript:void(0);">
                                                               <i class="material-icons">&#xE616;</i>
                                                               <?php echo __('Weekly Timesheet');?> <?php echo $this->Format->getlockIcon(1); ?>
                                                       </a>
                                               </li>
                                               <li>
												   <a onclick="trackEventLeadTracker('Quick Links','Daily Timesheet','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');showUpgradPopup(0,1);" href="javascript:void(0);">
														   <i class="material-icons">&#xE8DF;</i>
														   <?php echo __('Daily Timesheet');?> <?php echo $this->Format->getlockIcon(1); ?>
												   </a>
                                               </li>
                                            <?php }?>
                                    </ul>
													...
													<?php if($this->Format->isFeatureOn('gantt',$user_subscription['subscription_id'])) { ?>
														<li class="prevent_togl_li list-13 <?php if(CONTROLLER == "ganttchart" && (PAGE_NAME == "manage" || PAGE_NAME == "ganttv2")) { echo 'active'; } ?>">
															<a onclick="return trackEventLeadTracker('Quick Links','Gantt Chart','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');" href="javascript:showUpgradPopup(0,1);">
																<i class="left-menu-icon material-icons">&#xE919;</i>
																<?php echo __('Gantt Chart');?> <?php echo $this->Format->getlockIcon(1); ?>
															</a>
														</li>
													<?php }else{ ?>
                                                    <li class="prevent_togl_li list-13 <?php if(CONTROLLER == "ganttchart" && (PAGE_NAME == "manage" || PAGE_NAME == "ganttv2")) { echo 'active'; } ?>">
                                                        <a onclick="return trackEventLeadTracker('Quick Links','Gantt Chart','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');" href="<?php echo HTTP_ROOT.'ganttchart/manage';?>">
                                                                <!--<span class="os_sprite gct-icon"></span>-->
                                                                <i class="left-menu-icon material-icons">&#xE919;</i>
                                                                <?php echo __('Gantt Chart');?>
                                                        </a>
                                                    </li>
                                                    <?php } } ?>
                                    ...
                                    <li class="not-hide-li">
                                           <a href="javascript:void(0)" class="type-of-hd hdings">
                                            <strong><?php echo __("Company Settings");?></strong></a></li>	
                                                ...
                                                <li><a href="<?php echo HTTP_ROOT.'reminder-settings';?>" onclick="trackEventLeadTracker('Quick Links','Daily Catch-Up Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><i class="material-icons">&#xE003;</i> <?php echo __('Daily Catch-Up');?></a></li>
                                                ...
                                                <li><a href="<?php echo HTTP_ROOT.'labels';?>" onclick="trackEventLeadTracker('Quick Links','Manage Labels Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><i class="material-icons">label</i> <?php echo __('Manage Labels');?></a></li>
                                                ...
                                                <li><a href="<?php echo HTTP_ROOT.'LogTimes/resource_settings';?>" onclick="trackEventLeadTracker('Quick Links','Resource Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><i class="material-icons">&#xE7FB;</i><i class="material-icons abs_set_icon">&#xE8B8;</i> <?php echo __('Resource Setting');?></a></li>
                                                <?php  }else{?>
                                                ...
                                                <li <?php if(defined('COMP_LAYOUT') && COMP_LAYOUT){ ?> style="display:none;"<?php } ?>><a href="<?php echo HTTP_ROOT.'chat-settings';?>" onclick="trackEventLeadTracker('Quick Links','Chat Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><i class="material-icons">&#xE0B7;</i> <?php echo __('Chat Setting');?></a></li>
                                                <?php } ?>
                                        </ul>
                                    </li>
                                    <?php }else{ ?>
                        ...
                        <?php } ?>                                    
                            ...
                            <?php } ?>  
						<li class="feedback">
							<span data-toggle="modal" data-target="#feeback_modal" title="<?php echo __('Feedback');?>" rel="tooltip_down"><i class="material-icons">&#xE87F;</i></span>
						</li>
						<li class="help_navli">
                            <a href="<?php echo HTTP_ROOT;?>help-support">
							<span data-backdrop="false" onclick="trackEventLeadTracker('Header Link','Help','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Help');?></span></a>
						</li>
            <li class="nav-profile-dropdown nav-notification-bar quick-add-drop dropdown hover-menu cmn_parent_navli">
                            <a class="dropdown-toggle" data-target="#" href="javascript:void(0)" data-toggle="dropdown" onclick="seenNotification('total');trackEventLeadTracker('Header Link','Notification','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');">
                ...
                <ul class="dropdown-menu border-box notification-ul top_maindropdown-menu fadeout_bkp"></ul>
                        </li>
                        <?php } ?>
                        <li class="nav-profile-dropdown dropdown hover-menu cmn_parent_navli">
						<div class="gray-out-quick"></div>
                        <?php 
                        ...
                        ?>
                          <a href="javascript:void(0)" data-target="#" class="dropdown-toggle" data-toggle="dropdown">
                                <div class="prof_sett">
                                        ...
                                        <?php } ?>
                                        <span class="sett_icn fr">
											<i class="material-icons">&#xE8B8;</i>
                                        </span>
                                        <div class="cb"></div>
                                </div>
                          </a>
                          <div class="dropdown-menu top_maindropdown-menu cap-setting fadeout_bkp">
                              <div class="border-box">
                                <div>
                                    <p class="drop-paragraph"><?php echo __('This account is managed by');?> <span title="<?php echo CMP_SITE; ?>" class="account_mgt_by"><?php echo $this->Format->shortLength(CMP_SITE,25); ?></span></p>
                                        ...
                                        <?php if(SES_TYPE == 1 || SES_TYPE == 2 || IS_MODERATOR == 1) { ?>
                                            <li>
                                                ...
                                                <?php if(SES_TYPE == 1 || SES_TYPE == 2){ ?>
                                                    ...
                                                    <li><a href="<?php echo HTTP_ROOT.'reminder-settings';?>" onclick="trackEventLeadTracker('Top Setting','Daily Catch-Up Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Daily Catch-Up');?></a></li>
                                                    ...
                                                    <li><a href="<?php echo HTTP_ROOT.'labels';?>" onclick="trackEventLeadTracker('Quick Links','Manage Labels Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Manage Labels');?></a></li>
                                                    <li><a href="<?php echo HTTP_ROOT.'task-settings';?>" onclick="trackEventLeadTracker('Top Setting','Task Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Task Setting');?></a></li>
                                                    <?php if($this->Format->isResourceAvailabilityOn('upgrade')){ ?>
                                                     <li><a href="<?php echo HTTP_ROOT.'LogTimes/resource_settings';?>" onclick="trackEventLeadTracker('Quick Links','Resource Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Resource Setting');?></a></li>
                                                    <?php }else{ ?>
                                                    <li><a href="javascript:void(0);" onclick="trackEventLeadTracker('Quick Links','Resource Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');showUpgradPopup(0,1,1);"><?php echo __('Resource Setting');?> <img src="<?php echo HTTP_ROOT.'img/icon-upgrade.png'?>" style="width: 14px;float: right;margin-top: 2px;" title="<?php echo __('Upgrade Now');?>" /></a></li>
                                                    <?php } ?>
                                                    <?php if(SES_COMP==1 || SES_COMP==605 || SES_COMP==15270 || SES_COMP==18326 || SES_COMP==21723 || SES_COMP==24624 || SES_COMP==29320){?> 
                                                    <li <?php if(defined('COMP_LAYOUT') && COMP_LAYOUT){ ?> style="display:none;"<?php } ?>><a href="<?php echo HTTP_ROOT.'chat-settings';?>" onclick="trackEventLeadTracker('Top Setting','Chat Setting Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Chat Setting');?></a></li>
                                                    <?php } ?>
												...
												<li>
                                                 <a onclick="trackEventLeadTracker('Quick Links','Workflow Setting','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');" href="<?php echo HTTP_ROOT.'workflow-setting'; ?>" ><?php echo __('Status Workflow Setting');?></a>
                                            </li>
                                                <?php if((USER_TYPE == 1) || (IS_MODERATOR == 1)) { ?>
                                                    <li><a href="<?php echo HTTP_ROOT.'osadmins';?>" style="color:#FABC6A" target="_blank"><b><?php echo __('Super Admin');?></b></a></li>
                                                <?php } ?>
                                                </ul>
                                            </li>
                                        <?php } ?>
                                            ...
                                            <li>
                                                    <ul class="sub-menu hide_on_click">
                                                            <li class="not-hide-li">
                                                                    <div>
                                                                            <i class="material-icons">&#xE8B9;</i>
                                                                            <p><?php echo __('Account Settings');?></p>
                                                                    </div>
                                                            </li>
															...
															<li class="setng_subcription"><a href="<?php echo HTTP_ROOT.'pricing';?>" onclick="trackEventLeadTracker('Top Setting','Pricing Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Plan');?></a></li>
                                            ...
                                            <li>
                                                    <ul class="sub-menu hide_on_click">
                                                            <li class="not-hide-li">
                                                                    <div>
                                                                            <i class="material-icons">&#xE851;</i>
                                                                            <p><?php echo __('Personal Settings');?></p>
                                                                    </div>
                                                            </li>
                                                            ...
                                                            <?php if(TOT_COMPANY >= 2)  { ?>
                                                                <li><a href="<?php echo HTTP_ROOT.'users/launchpad';?>" onclick="return trackEventLeadTracker('Top Setting','Launchpad','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Launchpad');?></a></li>
                                                            <?php } ?>
															<li><a href="<?php echo HTTP_ROOT.'users/default_view';?>" onclick="trackEventLeadTracker('Top Setting','Default View Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Default View');?></a></li>
                                                            ...
                                                            <li><a href="<?php echo KNOWLEDGEBASE_URL; ?>" target="_blank" onclick="return trackEventLeadTracker('Top Setting','Help Desk Page','<?php echo $_SESSION['SES_EMAIL_USER_LOGIN'];?>');"><?php echo __('Help Desk');?></a></li>
                                                            <li><a href="<?php echo HTTP_ROOT;?>mobile-device"><?php echo __('Get free Mobile Apps');?></a></li>
															<li style="margin-bottom:5px;border-bottom:none;box-shadow: none;"><a href="<?php echo HTTP_ROOT;?>users/logout" class="sign_out"><?php echo __('Log Out');?></a></li>
                                                            </ul>
                                                    </li>
                                            </ul>
                                    </div>

<table cellpadding="0" cellspacing="0" width="100%" align="center" style="border:1px solid #DCDCDC;">
	<tr>
		<td style="color:#333333;" class="tophead"><?php echo __('Users of my projects');?> <font style="font-weight:normal">(<?php echo count($getUsers); ?>)</font></td>
	</tr>
	...
	<tr>
		<td style="border-top:0px solid #DCDCDC;padding-left:2px;" align="center">
			...
			<div style="float:left;padding:4px;width:45px">
				...
				{
				$random_bgclr = $this->Format->getProfileBgColr($usr['User']['id']);
				$usr_name_fst = mb_substr(trim($usr['User']['name']),0,1, "utf-8");				
				?>
				<span class="cmn_profile_holder <?php echo $random_bgclr; ?>" title="<?php echo $this->Format->formatText(trim($usr['User']['name']." ".$usr['User']['last_name'])); ?>">
				<?php echo $usr_name_fst; ?>
				</span>
				<?php
				}
				?>
...
<tr>
	<td style="border-top:1px solid #DCDCDC;padding:5px 2px;font-size:12px;" align="left">
		<div style="color:#777;text-align:center;"><?php echo __('No users found');?></div>
	</td>
</tr> 
<?php } ?>
</table>
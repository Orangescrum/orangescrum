	...
	<tr>
		<td style="padding-top:10px">
			<div id="show_temp_cases">
				...
				<tr height="28px">
						<td  style="padding-left:4px;" align="left" width="300px" class="tophead"><?php echo __('Name');?></td>
						<td  style="padding-left:4px;" align="left" width="80px" class="tophead"><?php echo __('Created By');?></td>
						<td  style="padding-left:4px;" align="left" width="80px" class="tophead"><?php echo __('Created On');?></td>
						<td align="center" width="55px" class="tophead"><?php echo __('Action');?></td>
					</tr>
					<?php
						$count=0; $clas = "";
						$totCase = 0; 
					...
					<tr class="<?php echo $clas?>" height="22px" id="templist<?php echo $count;?>" onmouseover="show_edit('<?php echo $count;?>');" onmouseout="hide_edit('<?php echo $count;?>');" <?php if($template['ProjectTemplate']['is_active'] == 0){?>style="background-color:#C8C8C8;"<?php }?>>
						<td style="padding-left:2px;">
							...
							<div style="display:none;margin-left:5px;" id="img_div<?php echo $count;?>">
								<img src="<?php echo HTTP_IMAGES; ?>images/del.gif" alt="Saving your data..." title="<?php echo __('Saving your data');?>..."/>
							</div>
							...
							<div style="float:left;position:relative;display:none;" id="edit_div<?php echo $count;?>" >
								<div style="float:left;position:absolute;left:10px;width:50px;">
									<div class="fl"><a href="javascript:void(0);" onclick="open_edit('<?php echo $count;?>');"><img src="<?php echo HTTP_ROOT.'img/images/comments.gif'?>"></div><div class="fl" style="margin-left:5px"><?php echo __('Edit');?></div></a>
								</div>
							</div>
						</td>
						<td style="padding-left:2px;">
						...
						?>
								...
								<div class="act_dis" rel=tooltip  title="<?php echo __('Deactivate');?>"></div>
								</a>&nbsp;
						<?php }else{ ?>
								<a href="javascript:void(0);" onClick="acttemplate(<?php echo $template['ProjectTemplate']['id']?>);">
								<div class="act_dis" rel=tooltip  title="<?php echo __('Activate');?>"></div>
								</a>&nbsp;
						<?php } ?>
								<a href="javascript:void(0);" onClick="deltemplate(<?php echo $template['ProjectTemplate']['id'];?>);">
									<div class="act_del" style="margin-left:5px;" rel="tooltip" title="<?php echo __('Delete');?>"></div>
								</a>
								<?php } ?>
								</div>
						</td>
					...
					<?php /*?><td colspan="4" align="center">
						...
						<td colspan="4" align="center">
							<a href="<?php echo HTTP_ROOT;?>projects/add_template/" style="text-decoration:none;"><div align="center" style="margin:10px;" class="button green"><?php echo __('Add new Template');?></button></div></a>
						</td>
					<?php
						}
					?>
...
function acttemplate(id)
{
	...
	var conf = confirm("<?php echo __('Are you sure to activate this template');?>?");
	if(conf == true) {
...
function inacttemplate(id)
{
	...
	var conf = confirm("<?php echo __('Are you sure to deactivate this template');?>?");
	if(conf == true) {
		var strURL = "<?php echo HTTP_ROOT?>";
...
function deltemplate(id)
{
	...
	var conf = confirm("<?php echo __('Are you sure to delete this template');?>?");
	if(conf == true) {
		var strURL = "<?php echo HTTP_ROOT?>";
...
function removecase(count,case_template_id,temp_name) {
	...
	var conf = confirm("<?php echo __('Are you sure you want to remove');?> ' "+temp_name+"' <?php echo __('from the template');?>?");
	if(conf == true) {
		var caseDiv = 'caseDiv'+count;
		var caseImg = 'caseImg'+count;
	...
	function save_edit_template(id,template_id){
		...
		if(temp_title.trim() != ''){
				...
				$.post(strURL,{"count":id,"template_id":template_id,"module_name":escape(temp_title)},function(data) {
					if(data){
						 if(data.trim() == 'fail') {
								...
								var op = 100;
								showTopErrSucc('error',"<?php echo __('Template name already exist');?>.");
								return false;
							  }else{
								$("#img_div"+id).hide();
								$('#val_div'+id).show();
			...
			$('#edit_template_value'+id).val(orig_temp_title);
			var op = 100;
			showTopErrSucc('error',"<?php echo __('Template name can\'t be blank');?>.");
			return false;
		}
	}
</script>

						...
						<a href="javascript:void(0);">
							<div class="fr" style="width:64px;">
								<?php if($totalfilter >3 && $limit1 >=3  ) {?>
									<span class="menu_arrow_prev" onclick="previousCustomFilter(<?php echo $limit1; ?>,'less');" title="<?php echo __('Previous');?>"> << </span>
								<?php } if($totalfilter >3 && ($limit1+3)< $totalfilter){ ?>
									<span class="menu_arrow_next" onclick="showmoreCustomFilter(<?php echo $limit1; ?>,'more');" title="<?php echo __('Next');?>"> >> </span>
								<?php } ?>	
							</div>
							<div class="cb"></div>
						</a>
...
<?php }else{ ?>
<li style="color:#FFFFFF;text-align:center;font-size:14px;"><?php echo __('No custom filters');?></li>
<?php } ?>

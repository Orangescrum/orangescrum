<?php if(isset($recent_milestones) && !empty($recent_milestones)) {
	...
	<div class="listdv">
	    ...
	    <div>
		<div class="fl status-dta"><?php if($is_overdue) {?><span class="fnt_clr_rd"><?php echo __('Overdue');?></span><?php } else {?><?php echo __('Due on');?> <?php echo $due_date; }?> <?php if($project=='all'){ ?><a href="<?php echo HTTP_ROOT."dashboard/?project=".$value['Project']['uniq_id'];?>"><?php echo $value['Project']['name'];?></a><?php }?></div>
		<div class="fr status-dta"><?php echo $value['0']['resolved'];?> of <?php echo $value['0']['totalcases'];?> <?php echo __('Tasks Resolved');?></div>
		<div class="cb"></div>
	    </div>
	</div>
    ...
    <div class="fnt_clr_rd"><?php echo __('No task group found');?>.</div>
<?php } ?>
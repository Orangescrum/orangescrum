<html>
<head>
	<title><?php echo __('Download Task');?></title>
<style type="text/css">
	body{
...
function sendDownloadTaskMail(obj){
	...
	$.post(url,{"dwnldUrl":"<?php echo $downloadurl;?>",'projName':"<?php echo $projName;?>",'caseNum':"<?php echo $caseNum;?>",'taskTitle':"<?php echo $taskTitle;?>",'zipfile':"<?php echo $zipfilename;?>"},function(res){
		if(res=='Success'){
			...
			$('#return_msg').val('<?php echo __("Mail send successfully");?>.');
		}else{
			$('#return_msg').removeClass('success').addClass('error');
			$('#return_msg').val('<?php echo __("Oops! We are not able to send email. Please try later");?>.');
		}
		obj.close();
	});
}
...
<?php }else{ ?>
<center>
	<div class="dwn-txt"><?php echo __('The compressed files are ready to be downloaded');?></div>

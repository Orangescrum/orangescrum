...
{
?>
	<h1 class="toplink"><?php echo __('Edit Task');?></h1>
<?php
}
else
{
?>
	<h1 class="toplink"><?php echo __('Add New Task');?></h1>
<?php
}
?>
<script>
	...
	$('#file_upload').fileUploadUI({
		...
		buildDownloadRow: function (file)
		{
			if(file.name != "error")
			{
				...
				{
					alert("<?php echo __('Error uploading file. File size cannot be more then 5 Mb');?>!");
				}
			}
			else
			{
				alert("<?php echo __('Error uploading file. Please try with another file');?>");
			}
		}
	});
});
	...
	<tr>
		<td align="left" valign="top">
			...
			<table cellpadding="0" cellspacing="10" border="0" style="border:0px solid #FF0000;">
						...
						<span id="leb_pri" <?php if($sel_mytype == 10) { echo "style='display:none'"; } ?>>
							...
							<select name="data[Easycase][priority]" id="sel_casePri" class="case_fields" style="width:80px">						  
								...
								?>
									...
									?>
									value="<?php echo $pri; ?>">
									<?php if($pri == 0) { echo _("High"); } elseif($pri == 1) { echo __("Medium"); } else { echo __("Low"); } ?>
									</option>
								<?php
								}
								?>
				...
				<tr>
					<td align="right" valign="top"><strong><?php echo __('Email Notification');?>:</strong></td>
					<td align="left" colspan="3" valign="top">
						<div id="ajxMemaddNewCase" style="display:none"><!--ajax_postcase_mem.ctp--></div>
						<div id="ajxMemaddNewCase2" style="display:block"><input type="checkbox" name="chk_all" id="chk_all" value="all" class="checkbox_style" onClick="checkedAllRes()" checked="checked">&nbsp;<?php echo __('All');?></div>
					</td>
				</tr>
				<tr>
					<td align="right" valign="top">
						<strong><?php echo __('Attachment');?>:</strong><br/><font style="font-size:12px;color:#008000;">(<?php echo __('optional');?></font>
					</td>
					<td align="left" colspan="3">
							...
							<tr>
								<td>
									<form id="file_upload" action="<?php echo HTTP_ROOT."easycases/fileupload/"; ?>" method="POST" enctype="multipart/form-data">
										...
										<span class="customfile-button button" aria-hidden="true"><?php echo __('Browse');?></span>
										<span class="customfile-feedback" aria-hidden="true"><?php echo __('Select files to upload');?>...</span>
										<input class="fileupload customfile-input" name="data[Easycase][case_files]" type="file" multiple="" />
										</div>
									</form>
								</td>
								<td style="padding-left:10px;" valign="top">
									<img src="<?php echo HTTP_IMAGES;?>images/attach.png" width="20" height="20" style="position:relative;top:4px;"/> <i> <?php echo __('Maximum file size allowed is 5 MB');?>.</i>
								</td>
							</tr>
							<tr>
				...
				<tr>
					<td align="right" valign="middle">
						<strong><?php echo __('Assign To');?>:</strong>
					</td>
					<td align="left" colspan="3">
						<table cellpadding="0" cellspacing="0" >
							<tr>
								<td>
									...
									<div id="ajaxCaseAsgnMe2" style="display:block;">
										<select name="data[Easycase][assign_to]" id="assign" class="case_fields">
											<option value=""><?php echo __('me');?></option>
										</select>
									</div>
								</td>
				...
				<tr>
					...
					<span id='sub_post'>
						...
						<button type="submit" value="Post" name="data[Easycase][postdata]" style="margin-left:3px;margin-top:5px;width:75px;padding:5px 5px 15px 5px;" class="blue small"><?php echo __('Post');?></button>
						or
						<a href="javascript:void(0);" class="nyroModalClose"><?php echo __('Cancel');?></a>
					</span>
					<span id='sub_loader' style="display:none;">
						<img src='<?php echo HTTP_IMAGES;?>images/del.gif' alt="Loading..." title="<?php echo __('Loading');?>..." width="16" height="16"/>
					</span>
					<?php
					}
					else
					...
					<span id='sub_post'>
						<button type="submit" value="Update & Continue" name="data[Easycase][postdata]" style="margin-left:3px;margin-top:5px;width:130px" class="blue small"><?php echo __('Update & Continue');?></button>
						<button type="submit" value="Update & View List" name="data[Easycase][postdata]" style="margin-left:3px;margin-top:5px;width:130px" class="blue small"><?php echo __('Update & View List');?></button>
						or
						<a href="javascript:void(0);" class="nyroModalClose"><?php echo __('Cancel');?></a>
					</span>
					<span id='sub_loader' style="display:none;">

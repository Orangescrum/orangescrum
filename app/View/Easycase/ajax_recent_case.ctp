	...
	<?php if($totalRecentCase){  ?>
				...
				<li title="<?php echo strtoupper($projarr['Project']['short_name']).' - '.$casearr['Easycase']['case_no']; ?>: <?php echo html_entity_decode($casearr['Easycase']['title']); ?>" <?php if($caseid == $casearr['Easycase']['uniq_id']) { echo "class='recent_current'"; } ?>>
						...
						<a href="javascript:void(0);">
							<div class="fr" style="width:64px;">
								<?php if($totalRecentCase >3 && $limit1 >=3  ) {?>
									<span class="menu_arrow_prev" onclick="previousRecentCase(<?php echo $limit1; ?>,'less');" title="<?php echo __('Previous');?>"> << </span>
								<?php } if($totalRecentCase >3 && ($limit1+3)< $totalRecentCase){ ?>
									<span class="menu_arrow_next" onclick="showmoreRecentCase(<?php echo $limit1; ?>,'more');" title="<?php echo __('Next');?>"> >> </span>
								<?php } ?>	
							</div>
							<div class="cb"></div>
						</a>

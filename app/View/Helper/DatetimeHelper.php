...
class DatetimeHelper extends AppHelper {
    ...
    function due_dateDiff($date1, $date2) {
        $date2 = explode(' ', $date2);
        $dStart = new DateTime($date2[0]);
        $dEnd = new DateTime($date1);
        $dDiff = $dStart->diff($dEnd);
        $days = $dDiff->format('%R%a');
        $day_ret = __('Today',true);
        if ($days > 0) {
            $day_ret = $dDiff->format('%a') . ' '.__('day(s) from today');
        } else if ($date1 == "0000-00-00 00:00:00") {
            $day_ret = __('Date Not Set',true);
        } else if ($days < 0) {
            $day_ret = __('Overdue',true);
        }
        return $day_ret;
    }
    ...
    function dateFormatOutputdateTime($date_time, $curdate = NULL, $type = NULL) {
        ...
        if ($diff_in_sec < 60) {
            ...
            if ($diff_in_min != 1) {
                return $diff_in_min . " ".__("mins ago",true);
            } else {
                return $diff_in_min . " ".__("min ago",true);
            }
        } else if ($diff_in_hr < 24) {
            if ($diff_in_hr != 1) {
                return $diff_in_hr . " ".__("hours ago",true);
            } else {
                return $diff_in_hr . " ".__("hour ago",true);
            }
        }
    }
    ...
    function facebook_style($date, $curdate = NULL, $type = NULL) {
        ...
        $periods = array(__("sec",true), __("min",true), __("hour",true), __("day",true), __("week",true), __("month",true), __("year",true), __("decade",true));
    ...
    function dateFormatOutputdateTime_details($date_time, $curdate) {
        if ($date_time != "") {
            ...
            if ($date_ex2[2] != "00") {
                ...
                if ($dateformated == $this->dateFormatReverse($curdate)) {
                    return __("Today at",true)." " . $timeformat;
                } else {

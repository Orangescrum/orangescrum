...
class CasequeryHelper extends AppHelper {
	...
	function getTaskTitle($cid,$typ,$case_no,$project_id)
	{
		...
		$Easycase->recursive = -1;
		if($typ == 2) {
			$getTitle = $Easycase->find('first', array('conditions' => array('Easycase.case_no' => $case_no,'Easycase.project_id' => $project_id,'Easycase.isactive' => 1,'Easycase.thread_count !=' => 0),'fields' => array('title')));
			return $getTitle['Easycase']['title'];
		}
		else {
	...
	function getProjectName($pid)
	{
		...
		$Project = ClassRegistry::init('Project');
		$Project->recursive = -1;
		$pjArr = $Project->find('first', array('conditions'=>array('Project.id' => $pid,'Project.company_id' => SES_COMP), 'fields'=>array('Project.name','Project.uniq_id','Project.short_name','Project.project_methodology_id')));

...
class TmzoneHelper extends AppHelper {
    ...
    function GetDateTime($timezoneid, $gmt_offset, $dst_offset, $timezone_code, $db_date, $type = 'datetime') {
        ...
        if ($type == "revdate") {
            ...
            if ($dst_offset > 0) {
                if (!($dst)) {
                    $dst_offset = 0;
                } else if (!$this->isDaylightSaving($timezoneid, $gmt_offset, $db_date)) {
                    $dst_offset = 0;
                }
            }
            $dst_offset *= 60;
    ...
    function isDaylightSaving($timezoneid, $gmt_offset, $db_date) {
        $gmt_minute = gmdate("i");
        $gmt_hour = gmdate("H");
        $gmt_month = gmdate("m");
        $gmt_day = gmdate("d");
        ...
        switch ($timezoneid) {
            ...
            case 19: /* 	Newfoundland */
                if ($this->afterFirstDayInMonth($cur_year, $cur_year, 3, "Sun", $gmt_offset, $db_date) &&
            ...
            case 7: /* 	Chihuahua, La Paz, Mazatlan */
                if ($this->afterFirstDayInMonth($cur_year, $cur_year, 5, "Sun", $gmt_offset, $db_date) &&
            ...
            case 18: /* 	Santiago, Chile */
                if ($this->afterSecondDayInMonth($cur_year, 10, "Sat", $gmt_offset, $db_date) &&
            ...
            case 20: /* 	Brasilia, Brazil */
                if ($this->afterFirstDayInMonth($cur_year, $cur_year, 11, "Sun", $gmt_offset, $db_date) &&
            ...
            case 23: /* 	Mid-Atlantic */
                if ($this->afterLastDayInMonth($cur_year, $cur_year, 3, "Sun", $gmt_offset, $db_date) &&
                        $this->beforeLastDayInMonth($cur_year, $cur_year, 9, "Sun", $gmt_offset, $db_date))
                    return true;
                else
                    return false;
            ...
            case 35: /* 	Cairo, Egypt */
                if ($this->afterLastDayInMonth($cur_year, $cur_year, 4, "Fri", $gmt_offset, $db_date) &&
            ...
            case 39: /* 	Baghdad, Iraq */
                if ($this->afterFirstOfTheMonth($cur_year, $cur_year, 4, $gmt_offset, $db_date) &&
            ...
            case 68: /* 	Canberra, Melbourne, Sydney */
                if ($this->afterLastDayInMonth($cur_year, $cur_year, 10, "Sun", $gmt_offset, $db_date) &&
            ...
            case 70: /* 	Hobart */
                if ($this->afterFirstDayInMonth($cur_year, $cur_year, 10, "Sun", $gmt_offset, $db_date) &&
                        $this->beforeLastDayInMonth($cur_year, $cur_year + 1, 3, "Sun", $gmt_offset, $db_date))
                    return true;
                else
                    return false;
    ...
    function afterFirstDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
        for ($i = 8; $i < 15; $i++) {
    ...
    function beforeLastDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
    ...
    function afterLastDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
        $days_in_month = $this->getDaysInMonth($month);
    ...
    function afterFirstOfTheMonth($curYear, $year, $month, $gmt_offset, $db_date) {
        $curDay = gmdate("d");
        $curMonth = gmdate("m");
        $curHour = gmdate("H") + $gmt_offset;
        /* The current time stamp */
    ...
    function beforeFirstOfTheMonth($curYear, $year, $month, $gmt_offset, $db_date) {
        $curDay = gmdate("d");
        $curMonth = gmdate("m");
        $curHour = gmdate("H") + $gmt_offset;
        /* The current time stamp */
    ...
    function beforeThirdDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
        $count = 0;
    ...
    function beforeSecondDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
        $count = 0;
    ...
    function afterSecondDayInMonth($curYear, $year, $month, $day, $gmt_offset, $db_date) {
        $count = 0;
